...
class GoogleMapAPI {
     ...
     * @param bool|int $geodesic render line as geodesic (Great Circle) or not (boolean or int)
     * @return bool|int Array id of newly added point or false
     */
    function addPolylineByCoordsArray($polyline_array,$id=false,$color='',$weight=0,$opacity=0,$geodesic=false){
    	...
    	foreach($polyline_array as $_coords){
          ...
          $_ln2=$_next_coords["long"];
    		  $id = $this->addPolyLineByCoords($_ln1, $_lt1, $_ln2, $_lt2, $id, $color, $weight, $opacity, $geodesic);
    		}
    	}
        return $id;
    }
     ...
     * @return bool|int Array id of newly added point or false
     */
    function addPolylineByAddressArray($polyline_array,$id=false,$color='',$weight=0,$opacity=0,$geodesic=false){
        if(!is_array($polyline_array) || sizeof($polyline_array) < 2)
           return false;
        $_prev_address = "";
        $_next_address = "";
        ...
        foreach($polyline_array as $_address){
            ...
            if($_prev_address !== ""){
              $id = $this->addPolyLineByAddress($_prev_address, $_next_address, $id, $color, $weight, $opacity, $geodesic);
            }
        }
        return $id;
    }
     ...
     * @return bool|int Array id of newly added point or false
     */
    function addPolyLineByAddress($address1,$address2,$id=false,$color='',$weight=0,$opacity=0,$geodesic=false) {
        if(($_geocode1 = $this->getGeocode($address1)) === false)
            return false;
        if(($_geocode2 = $this->getGeocode($address2)) === false)
            return false;
        return $this->addPolyLineByCoords($_geocode1['lon'],$_geocode1['lat'],$_geocode2['lon'],$_geocode2['lat'],$id,$color,$weight,$opacity,$geodesic);
    }
     ...
     */
    function addPolyLineByCoords($lon1,$lat1,$lon2,$lat2,$id=false,$color='',$weight=0,$opacity=0,$geodesic=false) {
		if($id !== false && isset($this->_polylines[$id]) && is_array($this->_polylines[$id])){
			$_polyline = $this->_polylines[$id];
		}else{
			//only set color,weight, opacity, and geodesic if new polyline
			$_polyline = array(
				"color"=>$color,
				"weight"=>$weight,
				"opacity"=>$opacity,
	...
	*/
    ...
    function getPolylineJS() {
        ...
        foreach($this->_polylines as $polyline_id =>$_polyline) {
          ...
          ".(($_polyline['geodesic']!="")?", geodesic: " . $_polyline['geodesic']."":"")."
			  });
    ...
    function getCreateMarkerJS() {
    	$_output = "
    	   function createMarker(map, point, title, html, icon, icon_shadow, sidebar_id, openers){
                ...
                if (window.lastInfo) window.lastInfo.close();
     ...
     * look in cache first, otherwise get from the designated
     * geocode service (GOOGLE by default)
     *
    ...
    /**
     * get geocode lat/lon points for given address from the specified geocode service (GOOGLE by default)
     *
     * @param string $address
